...
class ShopMessageScreen extends Component {
  constructor(props) {
    super(props);
  ...
  fetchData = async () => {
    ...
    try {
      ...
      const {data} = await getChatsForUser(user_id, shop_id);
      this.setState({
        user_info: data.user,
        chats: data.messages,
        loading: false,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getChats: (id) => dispatch(getShopChats(id)),
  };
};
const mapStateToProps = (state) => {
  return {
    ...
    chat: state.chat.list[state.chat.selected],
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(ShopMessageScreen);
