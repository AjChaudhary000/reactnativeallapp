...
export const fetchOnboardingUrl = async () => {
  ...
  return await GET(token, '/stripe/onboarding', {});
};
export const getLoginLink = async () => {
  const token = await getJWTToken();
  return await GET(token, '/stripe/loginlink', {});
};

export const checkOnboardingStatus = async () => {
  const token = await getJWTToken();
  return await GET(token, '/stripe/checkOnboardingStatus', {});
};
export const fetchPaymentParams = async (data) => {
  const token = await getJWTToken();
  return await POST(token, '/stripe/payment-sheet', data);
};
export const fetchTransferParams = async (data) => {
  const token = await getJWTToken();
  return await POST(token, '/stripe/transfer-intent', data);
};
