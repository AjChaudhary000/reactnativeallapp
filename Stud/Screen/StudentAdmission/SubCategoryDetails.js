...
function SubCategoryDetails({ navigation }) {
    const DeleteCategory=(cid)=>{
        var index = SubCategory.findIndex(item=>(item.id==cid))
        console.log(index)
        if(SubCategory.pop((SubCategory[index].id,SubCategory[index].category,SubCategory[index].sub_category))){
            alert("Delete data")
            navigation.goBack();
        }
       }
    const Data = SubCategory.map(item=>{return item })
    const renderdata=({item})=>{
        return(
            ...
            <View style={{flex:2,margin:2}}>
                <Text style={{fontSize:15,fontWeight:'bold',textAlign:'center'}}>{item.sub_category}</Text>
            </View>
            <View style={{flex:1,margin:2}}>
                <TouchableOpacity onPress={()=>navigation.navigate("AddSubCategory",{ id: item.id,type:'update'})}>
        ...
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <Animatable.View style={{ flexDirection: 'row', flex: 0.5 }}>
                ...
                <View style={{ marginTop: 10, height: 50, width: 220, justifyContent: 'center',backgroundColor:'gray',borderRadius:20 }}>
                    <Text style={{ textAlign: 'center', fontWeight: 'bold', letterSpacing: 2, fontSize: 25 }}>Sub-Category Details</Text>
                </View>
